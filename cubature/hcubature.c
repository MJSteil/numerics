 ...
 * @details Adaptive multidimensional integration on hypercubes (or, really, hyper-rectangles) using cubature rules.
 * A cubature rule takes a function and a hypercube and evaluates the function at a small number of points, returning
 * an estimate of the integral as well as an estimate of the error, and also a suggested dimension of the hypercube to
 * subdivide.
 *
 ...
 * Note, however, that we do not use any of code from the above authors (in part because their code is Fortran 77,
 * but mostly because it is under the restrictive ACM copyright license).  I did make use of some GPL code from Rudolf
 * Schuerer's HIntLib and from the GNU Scientific Library as listed in the copyright notice above, on the other hand.
 *
 * I am also grateful to Dmitry Turbiner <dturbiner@alum.mit.edu>, who implemented an initial prototype of the
 * "vectorized" functionality for evaluating multiple points in a single call (as opposed to multiple functions in a
 * single call).  (Although Dmitry implemented a working version, I ended up re-implementing this feature from scratch
 * as part of a larger code-cleanup, and in order to have a single code path for the vectorized and non-vectorized APIs.
 * I subsequently implemented the algorithm by Gladwell to extract even more parallelism by evalutating many hypercubes
 * at once.)
 *
 * TODO:
 * - Putting these routines into the GNU GSL library would be nice.
 * - A Python interface would be nice.  (Also a Matlab interface, a GNU Octave interface, ...)
 * - For high-dimensional integrals, it would be nice to implement a sparse-grid cubature scheme using Clenshaw-Curtis
 *   quadrature. Currently, for dimensions > 7 or so, quasi Monte Carlo methods win.
 * - Berntsen et. al also describe a "two-level" error estimation scheme that they claim makes the algorithm more
 *   robust.  It might be nice to implement this, at least as an option (although I seem to remember trying it once and
 *   it made the number of evaluations substantially worse for my test integrands).
 * Modified by Martin Jakob Steil in 2024:
 * - proper indentations
 * - some small adjustments to the code in terms of types and const-correctness
 *
 ...
 * - Incorporated implementation of converged from converged.h into this file
 * - Incorporated definition of fv_data struct from vwrapper.h into this file
 * - Incorporated declartation of fv wrapper from vwrapper.h into this file
 *
								 ...
								 unsigned nR, region *R)
{
   ...
   L. W. Fullerton, Bell Labs, Nov. 1981. */
	...
	for (iR = 0; iR < nR; ++iR) {
		...
		for (j = 0; j < (n - 1) / 2; ++j) {
			const unsigned j2 = 2*j + 1;
			double w = halfwidth * xgk[j2];
			pts[npts++] = center - w;
			pts[npts++] = center + w;
		}
		for (j = 0; j < n/2; ++j) {
			const unsigned j2 = 2*j;
			double w = halfwidth * xgk[j2];
			pts[npts++] = center - w;
			pts[npts++] = center + w;
		}
...
static void heap_resize(heap *h, size_t nalloc)
{
	h->nalloc = nalloc;
	if (nalloc)
		h->items = (heap_item *) realloc(h->items, sizeof(heap_item)*nalloc);// NOLINT(*-suspicious-realloc-usage)
	else {
					 ...
					 double reqAbsError, double reqRelError, error_norm norm)
{
	...
	unsigned j;
	#define SQR(x) ((x) * (x))
	switch (norm) { // NOLINT(hicpp-multiway-paths-covered)
						...
						double *val, double *err, int parallel)
{
	...
	while (numEval < maxEval || !maxEval) {
		...
		if (parallel) { /* maximize potential parallelism */
           ...
           realize that one could use a heap interval_data structure, in
           ...
           N, and it is also much simpler.] */
			...
			do {
				if (nR + 2 > nR_alloc) {
					nR_alloc = (nR + 2) * 2;
					R = (region *) realloc(R, nR_alloc * sizeof(region)); // NOLINT(*-suspicious-realloc-usage)
					if (!R) goto bad;
				}
...
typedef struct fv_data_s { integrand f; void *fdata; } fv_data; /* Incorporated definition from vwrapper.h */
...
static int fv(unsigned ndim, size_t npt, const double *x, void *d_, unsigned fdim, double *fval) {
	...
	for (i = 0; i < npt; ++i)
		if (f(ndim, x + i*ndim, fdata, fdim, fval + i*fdim))
			return FAILURE;
	return SUCCESS;
} /* Incorporated declaration from vwrapper.h */
